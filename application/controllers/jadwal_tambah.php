...
class jadwal_tambah extends CI_Controller 
{
	...
	public function index()
	{
		...
		$this->load->view('admin/v_jadwal_tambah',$data);
	}
	...
	public function insert_()
	{
		...
		if ($mapel == '' || $tanggal == '' || $jam == '' || $durasi_ujian == '' || $jenis == '') {
			...
			redirect(base_url('jadwal_tambah'));
		} else {
			$result = $this->m_data->insert_multiple();
			$this->session->set_flashdata('message', '<div class="alert alert-success alert-message"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><h4><i class="icon fa fa-check"></i> Jadwal Ujian berhasil dibuat !</h4></div>');
			redirect(base_url('jadwal_tambah'));
		}
	}	
}